...
class RequestBudgets extends Controller
{
    ...
    public function create($department_code = null)
    {
        ...
        return view('dashboard.admin.request.create', compact('department_code', 'departments', 'status', 'categories', 'users', 'budgets'));
    }
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('admin.budgets.requests.show', $budget['request_department'])->with('success', 'Budget added successfully');
    }
    ...
    public function show($department_code = null)
    {
        ...
        return view('dashboard.admin.request.show', compact('department_code', 'budgets', 'users', 'status', 'categories', 'requests'));
    }
    ...
    public function edit(string $id)
    {
        ...
        return view('dashboard.admin.request.edit', compact('budgets', 'departments', 'categories', 'status', 'requests', 'users', ));
    }
    ...
    public function update(Request $request, string $request_code)
    {
        ...
        return redirect()->route('admin.budgets.requests.show', $requests->request_department)->with('success', 'Budget updated successfully');
    }

    public function destroy($request_code)
    {
        $request = AddBudgets::findOrFail($request_code);

        // Manually delete associated records in add_budgets_request
        $request->delete();

        return redirect()->route('admin.budgets.requests.show', $request->request_department)->with('success', 'Request deleted successfully');
    }
    ...
    public function search(Request $request, $department_code = null)
    {
        ...
        if ($requests->isEmpty() && !is_null($department_code)) {
            ...
            return view('dashboard.admin.request.show', compact('department_code'))->with('error', 'No matching records found.');
        }

        return view('dashboard.admin.request.show', compact('requests', 'budgets', 'department_code', 'users', 'status', 'categories'));
    }
}
